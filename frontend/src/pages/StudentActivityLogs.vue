...
const fetchActivityLogs = async () => {
  ...
  try {
    ...
    const res = await httpClient.get(`/activity/student/${userId}`, {
...
<template>
  <div class="min-h-[calc(100vh-3.25rem)] bg-slate-50">
    <header class="sticky top-0 z-10 flex h-14 items-center gap-3 border-b border-slate-200 bg-white/95 px-4 sm:px-6 backdrop-blur">
      ...
      <div>
        <p class="text-[11px] uppercase tracking-[0.2em] text-slate-500">My Progress</p>
        <p class="text-sm font-semibold text-slate-900">Activity Log</p>
      </div>
      <button
    ...
    <main class="px-4 sm:px-6 pb-6 pt-4 sm:pt-5">
      ...
      <div v-else class="rounded-xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm">
        <h2 class="text-sm font-semibold text-slate-900 mb-1">Your Recent Activity</h2>
        <p class="text-xs text-slate-500 mb-4">{{ totalLogs }} total activity {{ totalLogs === 1 ? 'record' : 'records' }}</p>
